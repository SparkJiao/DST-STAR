...
class AttentionLayer(nn.Module):
    def __init__(self, attn: Attention, size, dropout, residual=False):
        super().__init__()
        self.attn = attn
...
class SlotAttentionLayer(nn.Module):
    ...
    def forward(self, x, value=None, mask=None):
        if value is not None:
            x = self.sublayer[0](x, lambda x: self.self_attn(x, value, value, mask))
        else:
